...
///Field `DCLRM` reader - Auto FIFO Buffer Clear Mode after Specified Pipe is Read
...
///Field `DCLRM` writer - Auto FIFO Buffer Clear Mode after Specified Pipe is Read
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum Rcnt {
    ///0: Clear DTLN\[11:0\] flags in the FIFO port control register to 0x000 when all receive data is read from DnFIFO (after read of a single plane in double buffer mode)
    _0 = 0,
    ///1: Decrement DTLN\[11:0\] flags each time receive data is read from DnFIFO
    _1 = 1,
}
...
impl R {
    ...
    ///Bit 13 - Auto FIFO Buffer Clear Mode after Specified Pipe is Read
    #[inline(always)]
